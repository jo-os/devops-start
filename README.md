...
mkdir ping-0.1; cd ping-0.1
cp ping-test.sh
export DEBEMAIL="joos@local"
export DEBFULLNAME="joos"
dh_make --indep --createorig
cd debian
rm *.ex
rm README.*
nano install - ping-test.sh /usr/bin
debuild -us -uc
cd ../../
dpkg -i ping_0.1-1_all.deb
sudo dpkg -r ping
```

**Источники информации об операционной системе**
...
- /proc/interrupts
- /proc/1/
  - cmdline - команда запуска (+ не всегда параметры)
  - cwd - ссылка на рабочую директорию
  - environ - переменные окружения
  - fd - файловые дискрипторы
  - mem,statm,maps - иформация о памяти предоставленной процессу
  - stat,status - информация о статусе процессора
- /proc/sys - тут можем менять параметры ядра на лету (до перезагрузки, /etc/sysctl.conf - сохранит параметры ядра)
...
- atop -r имя лога atop - открывает лог и tT - двигаемся по времени
- iotop
- iftop

**Мониторинг**
- USE - Utilisation, Saturation, Error
- RED - Rate, Error, Durtation
- LTES - Latency, Traffic, Error, Saturation

Построение алертинга основывается на значениях
- SLA - соглашение об уровне доступности - определяется как внешнее обязательство перед конечным пользователем или клиентом.
- SLO - набор целевых, желаемых значений SLI, выход за пределы которых может привести к нарушению SLA.
- SLI - набор ключевых метрик по которым можно определить жизненный статус сервиса, произодительность, удовлетворенность работой конечных пользователей.

**Бюджет ошибок** - максимально допустимое отклонение от идеальных показателей

**CPU**
- CPU idle < 10% в течении 10 минут
- CPU Load Average превышает количество доступных процессов и не равно 0 в течении 5 минут

**Memory**
- Заполнение SWAP > 90%
- Активная запись в SWAP в течении 2-5 минут (swapin > 1Мбит/с)
- Используемая оперативная память > 85%

**Disk**
- Нагрузка на диск (iostat) > 95% в течении часа
- Free inodes < 10%
- Свободное место на диске < 10% + дополнительсно < 5%
- Результат SMART-теста диска отличается от passed (failed - диск может быть неисправен)
- Количество поврежденных секторов HDD > 1
- Процент износа SSD < 10% от полезного срока службы
- Процент использования NVMe > 90%
- Объем резервной области NVMe < 15%
- Ошибки целостности данных NVMe > 1

**NTP**
- время ОС > 500 миллисекунд в течении 5 минут
- время ОС < 500 миллисекунд в течении 5 минут

**Триада CIA**
- Confidentiality (конфиденциальность) - доступ - минимальная необходимость
- Integrity (целостность) - защита от изменений (преднамеренных или случайных)
- Availability (доступность) - доступна когда необходима

**Методы защиты информации (нетехнические методы)**
- Правовые методы
